...
class StatsService {
  ...
  Future<Map<String, dynamic>> getStatsByPeriod({
    ...
    for (var con in consultations) {
      ...
      for (var p in listPaiements) {
        sommeTotale += (p['prix_a_paye'] as num).toDouble();
      }
  ...
  Future<Map<String, dynamic>> getStatsThisWeek() async {
    final now = DateTime.now();
  ...
  Future<Map<String, dynamic>> getStatsThisMonth() async {
    final now = DateTime.now();
    return await getStatsByPeriod(
